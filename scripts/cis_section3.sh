...
ensure_file() {
    ...
    if [ -e "$FILE_PATH" ]; then
        ...
        if [ "$CURRENT_OWNER" != "$TARGET_OWNER" ]; then
            ...
            echo "[OK] $FILE_PATH: Owner is correct ($CURRENT_OWNER)"
        fi
        ...
        if [ "$CURRENT_PERM" -gt "$TARGET_PERM" ]; then
            ...
            echo "[OK] $FILE_PATH: Perms $CURRENT_PERM is safe (Target: $TARGET_PERM)"
        else
            echo "[OK] $FILE_PATH: Perms is correct ($CURRENT_PERM)"
        fi
    else
